...
public:
  void filledGap(vector<int> &S, int &N) {
    ...
    for (int i = 0; i < N - 1; i++) {
      ...
      if (abs(difference) !=
          1) // if the difference is bigger than 1 fill the gap
      {
        int fill =
            difference > 0
                ? 1
                : -1; // calculates the difference between each adjacent num.
        for (int j = S[i] + fill; j != S[i + 1];
             j += fill) // dont get this part yet...
        {
          noGap.push_back(j);
        }
      }
